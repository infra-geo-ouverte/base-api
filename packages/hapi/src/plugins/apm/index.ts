...
export default (): IPlugin => {
  return {
    ...
    register: async (server: Hapi.Server, options: IPluginOptions) => {
      const apmOptions: ApmOptions | Record<string, never> =
        (options.apm as ApmOptions) || {};
      if (!apmOptions.name || !apmOptions.url) {
        server.logger.warn('APM options are required');
        return;
      }
        ...
        (request: Hapi.Request, h: Hapi.ResponseToolkit) => {
          ...
          if (apmOptions.pathReplace === true) {
            ...
            for (const param of matchParams) {
              path = path.replace(
                param,
                request.params[param.substring(1, param.length - 1)]
              );
            }
          }
          const routeApm = request.route.method.toUpperCase() + ' ' + path;
